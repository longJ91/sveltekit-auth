<script lang="ts">
	...
	import ImageComponent from '$lib/components/image/image-component.svelte';

	const isDesktop = mediaQuery('(min-width: 768px)');

	export let data: any;

	const items: Banner[] = data.bannerInfoResponse.banners;

	$: count = data.bannerInfoResponse.totalCount;
	$: page = 1;
	...
	const columns = table.createColumns([
		...
		table.column({
			...
			cell: ({ value }) => {
				return createRender(ImageComponent, {
					src: value,
					width: 100,
					height: 100
				});
	...
	const hideableCols = ['id', 'thumbnailUrl', 'linkUrl', 'status', 'createDate', 'updateDate'];
</script>
