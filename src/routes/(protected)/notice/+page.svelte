<script lang="ts">
	...
	const notices: Notice[] = data.noticeInfoResponse.notices;

	$: count = data.noticeInfoResponse.totalCount;
	$: page = 1;
	$: siblingCount = $isDesktop ? 1 : 0;

	const table = createTable(readable(notices), {
	...
	const columns = table.createColumns([
		...
		table.column({
			...
			accessor: 'content',
			cell: ({ value }) => {
				const formatted = value.length > 20 ? value.substring(0, 20) + '...' : value;
				return formatted;
			},
			plugins: { sort: { disable: true }, filter: { exclude: true } }
		}),
	...
	const hideableCols = ['id', 'title', 'content', 'createDate', 'updateDate'];
</script>
